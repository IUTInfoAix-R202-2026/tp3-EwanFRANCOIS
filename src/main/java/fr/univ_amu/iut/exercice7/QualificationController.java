...
public class QualificationController {
  ...
  private void initialize() {
    ...
    colHorodatage.setCellValueFactory(
        colHorodatage -> colHorodatage.getValue().horodatageProperty());
    // - colFrequence   -> c.getValue().frequenceDominanteKHzProperty()
    colFrequence.setCellValueFactory(
        colFrequence -> colFrequence.getValue().frequenceDominanteKHzProperty());
    // - colDuree       -> c.getValue().dureeSecondesProperty()
    colDuree.setCellValueFactory(colDuree -> colDuree.getValue().dureeSecondesProperty());
    // - colStatut      -> c.getValue().statutProperty()
    colStatut.setCellValueFactory(colStatut -> colStatut.getValue().statutProperty());
    ...
    labelVerdictGlobal
        .textProperty()
        .bind(
            Bindings.when(choiceBoxVerdict.valueProperty().isNull())
                .then("Verdict global : (à saisir)")
                .otherwise(Bindings.concat("Verdict global : ", choiceBoxVerdict.valueProperty())));
